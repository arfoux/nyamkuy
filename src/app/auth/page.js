...
export default function AuthPage() {
  ...
  const handleLogin = async () => {
    ...
    if (!email || !password) {
      setLoginMsg({ text: "Email dan password wajib diisi.", type: "error" });
      return;
    }
    ...
    try {
      ...
      if (!res.ok) {
        setLoginMsg({ text: data.error || "Terjadi kesalahan.", type: "error" });
      } else {
        setLoginMsg({ text: "Berhasil masuk! Mengalihkan...", type: "success" });
        setTimeout(() => router.push("/"), 800);
      }
    } catch {
      setLoginMsg({ text: "Tidak dapat terhubung ke server.", type: "error" });
    } finally {
  ...
  const handleRegister = async () => {
    ...
    if (!email || !password) {
      setRegMsg({ text: "Semua field wajib diisi.", type: "error" });
      return;
    }
    if (password.length < 8) {
      setRegMsg({ text: "Password minimal 8 karakter.", type: "error" });
      return;
    }
    if (password !== confirmPassword) {
      setRegMsg({ text: "Konfirmasi password tidak cocok.", type: "error" });
      return;
    }
    setRegLoading(true);
    try {
      ...
      const data = await res.json();
      if (!res.ok) {
        setRegMsg({ text: data.error || "Terjadi kesalahan.", type: "error" });
      } else {
        setRegMsg({ text: "Akun dibuat! Cek email kamu untuk verifikasi.", type: "success" });
        setRegDone(true);
      }
    } catch {
      setRegMsg({ text: "Tidak dapat terhubung ke server.", type: "error" });
    } finally {
      setRegLoading(false);
    }
  };
      ...
      <div className="w-full max-w-[400px] bg-background border border-border/60 rounded-2xl overflow-hidden shadow-sm">
        ...
        <div className="px-7 pt-6 pb-5 border-b border-border/50">
          <div className="flex items-center gap-2 mb-4">
            <div className="w-7 h-7 bg-foreground rounded-md flex items-center justify-center text-xs">
              🍲
            </div>
            <span className="text-sm font-semibold tracking-tight">Resepku</span>
          </div>
        ...
        <div className="flex border-b border-border/50">
          {["login", "register"].map((t) => (
            <button
              key={t}
              onClick={() => { setTab(t); setLoginMsg(null); setRegMsg(null); }}
